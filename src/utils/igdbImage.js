...
export default function getIgdbImage(input, options = {}) {
  ...
  const sizeToken = normalizeSize(options.size || options?.type || 't_cover_big');
  ...
  if (typeof input === 'object') {
    ...
    if (input.cover) {
      ...
      if (cover.url) {
        ...
        try { u = u.replace(/t_thumb/gi, sizeToken); } catch (e) {}
        return u;
      }
    }
    ...
    const candidates = [
      ...
      (input.screenshots && input.screenshots[0] && input.screenshots[0].image_id),
    ];
    ...
    for (const cand of candidates) {
      ...
      if (typeof cand === 'string') {
        let u = cand;
        if (u.startsWith('//')) u = 'https:' + u;
        if (!/^https?:\/\//i.test(u) && /^[a-z0-9_]+$/i.test(u)) u = buildFromId(u, sizeToken);
        try { u = u.replace(/t_thumb/gi, sizeToken); } catch (e) {}
        if (u) return u;
